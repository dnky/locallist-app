        ...
        <div id="page-content-full-width">
          ...
          <div className={`listings-container ${viewMode === 'map' ? 'map-view' : ''} ${viewMode === 'map' ? 'mobile-map-view' : 'mobile-list-view'}`}>
                    ...
                    <div className="business-listing" key={ad.id}>
                      ...
                      <div className="listing-content">
                        ...
                        <div className="listing-contact-mobile">
                          {ad.phone && (<a href={`tel:${ad.phone}`} onClick={(e) => e.stopPropagation()}><i className="fa-solid fa-phone"></i> {ad.phone}</a>)}
                          {ad.email && (<a href={`mailto:${ad.email}`} onClick={(e) => e.stopPropagation()}><i className="fa-solid fa-envelope"></i> {ad.email}</a>)}
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            <div className="map-panel">
              {/* --- REPLACE PLACEHOLDER WITH DYNAMIC MAP --- */}
              <DynamicMap ads={adsToDisplay} />
            </div>
          </div>
        </div>
